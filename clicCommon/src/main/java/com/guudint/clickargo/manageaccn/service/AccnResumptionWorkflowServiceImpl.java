...
@Service("AccnResumptionWorkflowServiceImpl")
public class AccnResumptionWorkflowServiceImpl extends AbstractWorkflowService<TCoreAccn, CoreAccn> {

	private static final Logger LOG = Logger.getLogger(AccnResumptionWorkflowServiceImpl.class);
	...
	public CoreAccn moveState(FormActions action, CoreAccn dto, Principal principal, ServiceTypes serviceTypes)
		...
		try {
			...
			str.append(" and o.wkflId LIKE 'ACNRESUMPT%'");
			...
			char newState = CoreAccnStateEnum
					.getStateByAltCodeAndDesc(toStateWf.getTCkMstJobStateByWkflToState().getJbstId(), "RESUMPT");
			...
			switch (action) {
			case RESUMPTION:
				event = "ACCOUNT RESUMPTION SUBMITTED";
				break;
			case APPROVE:
				event = "ACCOUNT RESUMPTION APPROVED";
				break;
			case DELETE:
				event = "ACCOUNT RESUMPTION DELETED";
				break;
			case REJECT:
				event = "ACCOUNT RESUMPTION REJECTED";
				break;
			default:
				break;
