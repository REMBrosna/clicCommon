...
@Service("AccnTerminationWorkflowServiceImpl")
public class AccnTerminationWorkflowServiceImpl extends AbstractWorkflowService<TCoreAccn, CoreAccn> {

	private static final Logger LOG = Logger.getLogger(AccnTerminationWorkflowServiceImpl.class);
	...
	public CoreAccn moveState(FormActions action, CoreAccn dto, Principal principal, ServiceTypes serviceTypes)
		...
		try {
			...
			str.append(" and o.wkflId LIKE 'ACNTER%'");
			...
			char newState = CoreAccnStateEnum
					.getStateByAltCodeAndDesc(toStateWf.getTCkMstJobStateByWkflToState().getJbstId(), "TER");
			...
			switch (action) {
			case TERMINATE:
				event = "ACCOUNT TERMINATION SUBMITTED";
				break;
			case APPROVE:
				event = "ACCOUNT TERMINATION APPROVED";
				break;
			case DELETE:
				event = "ACCOUNT TERMINATION DELETED";
				break;
			case REJECT:
				event = "ACCOUNT TERMINATION REJECTED";
				break;
			default:
				break;
