...
@Service("AccnSuspensionWorkflowServiceImpl")
public class AccnSuspensionWorkflowServiceImpl extends AbstractWorkflowService<TCoreAccn, CoreAccn> {

	private static final Logger LOG = Logger.getLogger(AccnSuspensionWorkflowServiceImpl.class);
	...
	public CoreAccn moveState(FormActions action, CoreAccn dto, Principal principal, ServiceTypes serviceTypes)
		...
		try {
			...
			str.append(" and o.wkflId LIKE 'ACNSUS%'");
			...
			char newState = CoreAccnStateEnum
					.getStateByAltCodeAndDesc(toStateWf.getTCkMstJobStateByWkflToState().getJbstId(), "SUS");
			...
			switch (action) {
			case SUSPEND:
				event = "ACCOUNT SUSPENSION SUBMITTED";
				break;
			case APPROVE:
				event = "ACCOUNT SUSPENSION APPROVED";
				break;
			case DELETE:
				event = "ACCOUNT SUSPENSION DELETED";
				break;
			case REJECT:
				event = "ACCOUNT SUSPENSION REJECTED";
				break;
			default:
				break;
