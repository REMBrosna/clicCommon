package com.guudint.clickargo.security.controller;
...
/**
 * Clickargo main auth service controller.
 */
@RestController("clickargoAuthController")
@RequestMapping(value = "/api/v1/clickargo/auth")
public class ClickargoAuthServiceController extends ServiceController {

	// Static Attributes
	////////////////////
	private static Logger log = Logger.getLogger(ClickargoAuthServiceController.class);
	...
	@Autowired
	@Qualifier("clickargoAuthService")
	private IAuthorize ckAuthorizeService;
	...
	public ResponseEntity<Object> permMenu() {
		...
		try {
			...
			List<PermExMenu> permExMenus = ckAuthorizeService.getPermMenu(principal, true);
			...
			// return ResponseEntity.ok(permExMenus);
		} catch (Exception ex) {
