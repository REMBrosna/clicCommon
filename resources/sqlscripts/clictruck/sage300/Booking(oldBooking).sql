...
-- CO DN
select 'clicTruck' as SERVICE,'book' as 'SAGE_TYPE', job.JOB_ID as `REFERENCE`, DATE_FORMAT(job.RCD_DT_BILL_APPROVED, '%Y-%m-%dT%T') as 'DATETIME', 'Consumer' as 'TYPE_ID'
    ...
    , '' as 'TAX_NO', round(dn.DN_TOTAL) as 'TOTAL', job.RCD_DT_BILL_APPROVED
from job, accn as accnCO, T_CK_CT_DEBIT_NOTE dn
...
union
-- TO DN
select 'clicTruck' as service,'book' as 'type', job.JOB_ID as `reference`, DATE_FORMAT(job.RCD_DT_BILL_APPROVED, '%Y-%m-%dT%T') as 'dateTime', 'Provider' as 'typeid'
	, accnTO.ACFG_VAL as 'Cust./ProvicerId', 'DebitNote' as 'Type', dn.DN_NO as 'DocNo', DATE_FORMAT(dn.DN_DT_ISSUE,'%Y-%m-%d') as 'issuedDate'
    , 'IDR' as 'ccy', round(dn.DN_AMT) as 'amount', 0 as 'vat', 0 as 'duty', concat('N',datediff(DN_DT_DUE, DN_DT_ISSUE)) as terms
    , '' as 'taxNo', round(dn.DN_TOTAL) as 'total', job.RCD_DT_BILL_APPROVED
from job, accn as accnTO, T_CK_CT_DEBIT_NOTE dn
...
union
-- CO Invoice
select 'clicTruck' as service,'book' as 'type', job.JOB_ID as `reference`, DATE_FORMAT(job.RCD_DT_BILL_APPROVED, '%Y-%m-%dT%T') as 'dateTime', 'Consumer' as 'typeid'
    ...
    , INV_SAGE_TAX_NO as 'taxNo', round(inv.INV_TOTAL) as 'total', job.RCD_DT_BILL_APPROVED
from job, accn as accnCO, T_CK_CT_PLATFORM_INVOICE inv
where job.JOB_PARTY_CO_FF = accnCO.ACCN_ID
...
select 'clicTruck' as service,'book' as 'type', job.JOB_ID as `reference`, DATE_FORMAT(job.RCD_DT_BILL_APPROVED, '%Y-%m-%dT%T') as 'dateTime', 'Provider' as 'typeid'
    ...
    , 'IDR' as 'ccy', round(-inv.INV_AMT) as 'amount', round(-INV_VAT) as 'vat', round(-INV_STAMP_DUTY) as 'duty', concat('N',datediff(INV_DT_DUE, INV_DT_ISSUE)) as terms
    , INV_SAGE_TAX_NO as 'taxNo', round(-inv.INV_TOTAL) as 'total', job.RCD_DT_BILL_APPROVED
from job, accn as accnTO, T_CK_CT_PLATFORM_INVOICE inv
where job.JOB_PARTY_TO = accnTO.ACCN_ID
